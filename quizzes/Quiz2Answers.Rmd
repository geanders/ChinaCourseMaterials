...
```{r echo = FALSE}
library(knitr)
knitr::opts_chunk$set(fig.path='quiz3/plot-')
```
...
![](../../RCourseGradedMaterials/Quizzes/figures/FileDirectoryStructure.png)
...
Which of the following is a **good** reason for saving an R object you've created using the `save()` command?

a. You want to make sure that your object is easy for later researchers to open and read, even if they don't have access to R
b. There are no good reasons for ever saving an R object using `save()`
c. The object is really small, so it's not worth the effort of reading it in from a flat file, like a `.csv`, every time you want to use it
d. You've created an object, like a model object, that doesn't have a 2-D structure (i.e., rows and columns), so it doesn't fit well into a flat file, but you want to share it with other people

> **Answer d.** 
> Explanation: It is very tricky to save things that don't have a 1-D (i.e., vector) or 2-D (i.e., dataframe or matrix) structure as flatfiles. If your datafile is small (Answer c.), your computer can read it into R very quickly, so for the sake of fully reproducible code, it's useful to read it in directly from the flat file (e.g., csv) every time. R files (`.RData`) don't make any sense if you open them with a text editor; they only make sense if you load them from R. Therefore, if later researchers did not have access to R software, they may not be able to read any files saved as `.RData` files (Answer a.).

##### 13.

Which of the following vector classes is the only one that R does **not**, at some level, store using numerical values?
...
> **Answer c.**
> Explanation: R stores factors (Answer a.) underneath as numbers. For example, try `as.numeric(factor(c("yes", "no", "yes")))`. It is important to know this, because occasionally R will do something that you didn't expect it to with a factor, and the underlying numerical structure is often the culprit. R stores date (Answer b.) as numerics of the number of days since some origin date (Jan 1, 1970, I think). R stores logical vectors (Answer d.) as FALSE = 0 and TRUE = 1.
