...
For today's class, you'll be using a dataset of all the guests on Jon Stewart's *The Daily Show*. This data was originally collected by Nate Silver's website, [FiveThirtyEight](http://fivethirtyeight.com) and is available on [FiveThirtyEight's GitHub page](https://github.com/fivethirtyeight/data/tree/master/daily-show-guests). The only change made to the original file was to add (commented) attribution information at the start of the file.

# Getting the data onto your computer

* First, create a new R script file to put all the code you use for this exercise.
* Next, you should have downloaded all the course materials, which include this data, from GitHub onto your computer. If you have not, then go to [the course GitHub repository](https://github.com/geanders/ChinaCourseMaterials) and click on "Download ZIP". This may take a few minutes, but it will download a compressed folder to your computer with all the course materials inside. Un-zip the file to get access to all the files (Mac computers will automatically do this when you click on the file). 
...
* Outside of R, open Finder (or your system's equivalent). Go to your home directory (mine, for example, is `/Users/brookeanderson`). Figure out home to get from this to the directory you downloaded from GitHub for this course. For example, from my home directory, I would go `Desktop` to `ChinaCDCRWorkshop` to `ChinaCourseMaterials`. 
* Go back into R. Set R's working directory to your directory for this class using the `setwd()` command, now that you know the pathname for the directory. For example, I would use `setwd("Desktop/ChinaCDCRWorkshop/ChinaCourseMaterials")`. 
* Use the `list.files()` command to make sure that the `daily_show_guests.csv` file is in the `data` subdirectory of your current working directory.
...
setwd("Desktop/ChinaCDCRWorkshop/ChinaCourseMaterials")
getwd()

list.files()
list.files("data")

"daily_show_guests.csv" %in% list.files("data")
```
...
Example R code:
```{r}
daily_show_first10 <- read.csv("data/daily_show_guests.csv",
                       header = TRUE,
                       skip = 4,
                       nrows = 10)
daily_show_first10
