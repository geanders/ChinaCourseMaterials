---
title: "Week 4- In Course Exercise"
...
library(knitr)
library(ggthemes)
knitr::opts_chunk$set(fig.path='classexercise/plot-')
```
...
- Use some of the plotting options to make this plot a bit more attractive. 
- Plot the same relationship using ggplot
...
with(worldcup, plot(Time, Shots))
```

Use some of the plotting options to improve the graph's appearance:

```{r, fig.width = 5, fig.height = 4}
with(worldcup, plot(Time, Shots, cex = 0.5, 
                    xlab = "Minutes played", 
                    ylab = "Shots on goal", 
                    main = "Time played vs. shots taken\n World Cup 2010"))
```

Create a plot of the same relationship using ggplot:
